...
def restruct(file_num):
    try:
        ...
        date = re.split('[T/+]',df['UpdateTime'][0])[0]
        week = datetime.datetime.strptime(date, '%Y-%m-%d').weekday()+1
        clock = file_num.split('_')[3]

    except:
        print(f"{Colorfill.FAIL}Bad operation finding {file_num}.{Colorfill.RESET}")
        return
    ...
    for location,lot_num,id in zip(name,spaces,parklot_id):
        ...
        if(lot_num==-1):  # case with bad data
            ...
            try:
                with open(f'{os.getcwd()}//data//data_storage//Parklot_Available//proceeded_data//{id}.json','r+',encoding='utf-8') as f:
                    base_file = json.load(f)
                    base_file[f'{week}-{clock}'].update({'current_space': lot_num})
                    ...
                    base_file[f'{week}-{clock}'].update({'dataset_quantity':base_file[f'{week}-{clock}']['dataset_quantity'] + 1})
                    f.truncate(0)
                    f.seek(0)
                    json.dump(base_file,f)
                #fb.put(f'parklot_available/{id}/',f'{week}-{clock}',{'current_space':lot_num,'avg_space':base_file[f'{week}-{clock}']['avg_space'],'dataset_quantity':base_file[f'{week}-{clock}']['dataset_quantity']})
                print(f"{Colorfill.OK}Finish {Colorfill.WARNING}{location}({id})|{week}-{clock}{Colorfill.RESET}")
                ...
                try:
                    #fb.put(f'parklot_available/{id}/',f'{week}-{clock}',{'current_space':lot_num,'avg_space':lot_num,'dataset_quantity':1})
                    with open(file = f'{os.getcwd()}//data//data_storage//Parklot_Available//proceeded_data//{id}.json',mode = 'r+',encoding='utf-8') as f:
                        base_file = json.load(f)
                        base_file[f'{week}-{clock}']=({"current_space": lot_num, "avg_space": lot_num, "dataset_quantity": 1})
                        f.truncate(0)
                        f.seek(0)
                        json.dump(base_file,f)
                    ...
                    #fb.put(f'parklot_available/{id}/',f'{week}-{clock}',{'current_space':lot_num,'avg_space':lot_num,'dataset_quantity':1})
                    #fb.put(f'parklot_available/{id}/','name', location)
                    with open (file = f'{os.getcwd()}//data//data_storage//Parklot_Available//proceeded_data//{id}.json',mode = 'w+',encoding='utf-8') as f:
                        base_file=dict()
                        base_file[f'{week}-{clock}']={"current_space": lot_num, "avg_space": lot_num, "dataset_quantity": 1}
                        base_file.update({"name":location})
                        json.dump(base_file,f)
...
if __name__ == '__main__':
    ...
    for i in num_list[2500:3899]:  #3899
        restruct(i)
